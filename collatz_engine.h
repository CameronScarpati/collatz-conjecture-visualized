...
private:
  ...
  applyCollatzFormula(unsigned long long num) {
    return (num % 2 == 0) ? (num / 2) : (3 * num + 1);
  }

public:
  // Compute (or return from cache) the Collatz sequence for a given starting
  // number.
  const std::vector<unsigned long long> &getSequence(unsigned long long start) {
    if (start < 1)
      throw std::runtime_error(
          "CollatzEngine: Only positive integers allowed.");
