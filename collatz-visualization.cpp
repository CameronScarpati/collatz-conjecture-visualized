...
std::unordered_map<unsigned long long, std::vector<unsigned long long>> collatzBranches;
...
// Animation control
unsigned long long currentBranch = 1;  // In selective mode, this is an index into selectedBranches.
...
void computeAxisLimits() {
    ...
    int exponent = (maxValue > 10.0f) ? static_cast<int>(std::floor(std::log10(maxValue))) : 0;
    verticalAxisPlacement = (exponent > 0) ? -1.1f + (float) exponent * 0.013f : -1.1f;
}
...
float scaleX(int step) {
    ...
    return verticalAxisPlacement + (static_cast<float>(step) / (maxIterations - 1.0f)) * graphWidth;
}
...
void hsvToRgb(float H, float S, float V, float &r, float &g, float &b) {
    ...
    r = rPrime + m; g = gPrime + m; b = bPrime + m;
}

void getRainbowColor(int step, float &r, float &g, float &b) {
    float ratio = (maxIterations > 1.0f) ? static_cast<float>(step) / (maxIterations - 1.0f) : 0.0f;
...
void drawAnimatedBranch(const std::vector<unsigned long long>& values, unsigned long long branchIndex) {
    // Glow pass
    glLineWidth(8.0f);
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    glBegin(GL_LINE_STRIP);
    for (int i = 0; i <= branchIndex && i < static_cast<int>(values.size()); i++) {
        float r, g, b;
...
void drawPrompt() {
    glColor3f(1.0f, 1.0f, 1.0f);
    if (selectBranches)
        drawText(-0.5f, 0.0f, "Enter branch numbers (comma separated): " + inputBuffer);
    else
...
unsigned long long getPartialPeak(const std::vector<unsigned long long>& values, unsigned long long branchIndex) {
...
void drawStatistics() {
    ...
    if (!animationDone) {
        if (selectBranches && currentBranch < selectedBranches.size()) {
            ...
            unsigned long long partialPeak = getPartialPeak(collatzBranches[branchNum], currentIndex);
            ...
            unsigned long long partialPeak = getPartialPeak(collatzBranches[currentBranch], currentIndex);
            glColor3f(1.0f, 1.0f, 0.0f);
    ...
    if (selectBranches) {
        ...
        unsigned long long avgSteps = (branchesDone > 0) ? sumSteps / branchesDone : 0;
        ...
        for (const auto &p : collatzStatsMap) {
            ...
            overallStats.overallMaxPeak = std::max(overallStats.overallMaxPeak, p.second.branchPeak);
        }
        unsigned long long avgSteps = (overallStats.branchesDone > 0) ? overallStats.sumOfSteps / overallStats.branchesDone : 0;
        glColor3f(0.0f, 1.0f, 0.0f);
        std::string msg = "Branches=" + std::to_string(overallStats.branchesDone)
...
void drawBottomInfo() {
    glColor3f(1.0f, 1.0f, 1.0f);
    drawText(-0.7f, -1.10f, "Current animation delay: " + std::to_string(ANIMATION_DELAY_MS) + " ms");
    drawText(-0.7f, -1.15f, std::string("Using ") + (useYLogScale ? "log" : "linear") + "-scale for Y-axis.");
    if (instantRender) {
